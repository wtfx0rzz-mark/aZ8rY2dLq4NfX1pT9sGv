...
return function(C, R, UI)
    local function run()
        ...
        local function disconnectSwing()
            ...
            if swingConnAnim then swingConnAnim:Disconnect() swingConnAnim = nil end
        end
        ...
        local function bindAnimator(ch)
            if swingConnAnim then swingConnAnim:Disconnect() swingConnAnim = nil end
            if not ch then return end
        ...
        local function setupSwing()
            ...
            swingConnInput = UIS.InputBegan:Connect(function(input, gameProcessed)
                if not swingEnabled then return end
            ...
            local function onCharacter(ch)
                ...
                swingConnChildAdded = ch.ChildAdded:Connect(function(inst)
                    ...
                    if inst and inst:IsA("Tool") then
                        task.wait()
                        if equippedTool() == inst then
                            bindToolActivated(inst)
                        end
                    end
                end)
        ...
        tab:Toggle({
            Title = "Swing Hits Trees (Radius 75)",
            Value = false,
            Callback = function(state)
