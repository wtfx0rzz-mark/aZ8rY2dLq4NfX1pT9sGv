return function(C, R, UI)
    local function run()
        ...
        local function computeImpactCFrameSimple(hitPart)
        ...
        local SwingTreeImpactCF = setmetatable({}, { __mode = "k" })
        local SwingTreeHitSeed = setmetatable({}, { __mode = "k" })
        local function swingImpactCFForTree(treeModel, hitPart)
            local base = SwingTreeImpactCF[treeModel]
            if not base then
                base = computeImpactCFrameSimple(hitPart)
                SwingTreeImpactCF[treeModel] = base
            end
            local k = (SwingTreeHitSeed[treeModel] or 0) + 1
            SwingTreeHitSeed[treeModel] = k
            return jittered(base, k)
        end
        ...
        local swingConnActivated = nil
        local swingConnChar = nil
        local swingConnChildAdded = nil
        local lastSwingAt = 0
        local SWING_DEBOUNCE = 0.08
        local SWING_MAX_TREES = 60
        local function getOriginAndRadius()
            local ch = lp.Character
            if not ch then return nil, nil end
            local head = ch:FindFirstChild("Head")
            if head and head:IsA("BasePart") then return head.Position, CHOP_RADIUS end
            local hrp = ch:FindFirstChild("HumanoidRootPart")
            if hrp and hrp:IsA("BasePart") then return (hrp.Position + Vector3.new(0, 2.5, 0)), CHOP_RADIUS end
            return nil, nil
        end
        ...
        local function isAxeLike(tool)
            ...
            if type(n) == "string" and n:find("Axe", 1, true) then return true end
            return false
        end
        local function swingHitNearbyTrees(tool)
            if not swingEnabled then return end
            if not tool or not tool.Parent then return end
            if not isAxeLike(tool) then return end
            local now = os.clock()
            if (now - lastSwingAt) < SWING_DEBOUNCE then return end
            lastSwingAt = now
            local origin, radius = getOriginAndRadius()
            if not origin then return end
            local trees = collectTreesInRadius(origin, radius)
        ...
        local function disconnectSwingTool()
            if swingConnActivated then swingConnActivated:Disconnect() swingConnActivated = nil end
        end
        local function bindToolActivated(tool)
            disconnectSwingTool()
            if not tool or not tool:IsA("Tool") then return end
            swingConnActivated = tool.Activated:Connect(function()
                swingHitNearbyTrees(tool)
            end)
        end
        local function rescanEquippedTool()
            local t = equippedTool()
            if t then bindToolActivated(t) else disconnectSwingTool() end
        end
        local function teardownSwingMonitoring()
            disconnectSwingTool()
        ...
        local function setupSwingMonitoring()
            teardownSwingMonitoring()
        ...
        tab:Toggle({
            Title = "Swing Hits Nearby Trees",
            Value = false,
            Callback = function(state)
                swingEnabled = (state == true)
                if swingEnabled then
                    lastSwingAt = 0
                    setupSwingMonitoring()
                    rescanEquippedTool()
                else
                    teardownSwingMonitoring()
                end
