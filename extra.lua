return function(C, R, UI)
    ...
    ExtraTab:Toggle({
        ...
        Callback = function(on)
            ...
            if on then
                startRifleZeroReloadreturn function(C, R, UI)
    C  = C  or _G.C
    UI = UI or _G.UI
    assert(C and UI and UI.Tabs and UI.Tabs.Extra, "extra.lua: missing context or Extra tab")
    ...
    local QUICK_OPEN_SETTLE_WAIT = 0.05
    local QUICK_OPEN_STEP_DELAY = 0.04
    local QUICK_COLLECT_TELEPORT_SETTLE = 0.05
    local QUICK_COLLECT_STEP_DELAY = 0.04
    local QUICK_PASS_GAP_DELAY = 0.10
    local QUICK_EMPTY_WAIT = 1.0
    ...
    local function quickOpenPass()
        ...
        while alive and quickOn and trackOn do
            ...
            local okTp = teleportNearChest(chest)
            if okTp then
                if QUICK_OPEN_SETTLE_WAIT > 0 then task.wait(QUICK_OPEN_SETTLE_WAIT) end
            ...
            removeTrackedChest(chest)
            currentRunChest = nil
            if QUICK_OPEN_STEP_DELAY > 0 then task.wait(QUICK_OPEN_STEP_DELAY) end
        end
    end
    ...
    local function quickCollectPass()
        ...
        for i=1,#QuickVisited do
            ...
            if pos then
                local cf = CFrame.new(pos + Vector3.new(0, STAND_UP, 0), pos)
                teleportToCF(cf)
                if QUICK_COLLECT_TELEPORT_SETTLE > 0 then task.wait(QUICK_COLLECT_TELEPORT_SETTLE) end
                processAllNear(pos, captureRadius, CHEST_CAPTURE_MAX_PER_POLL)
                if QUICK_COLLECT_STEP_DELAY > 0 then task.wait(QUICK_COLLECT_STEP_DELAY) end
            end
        end
        table.clear(QuickVisited)
    end
    ...
    local function startQuickChests()
        ...
        quickLoop = task.spawn(function()
            while alive and quickOn do
                quickOpenPass()
                if QUICK_PASS_GAP_DELAY > 0 then task.wait(QUICK_PASS_GAP_DELAY) end
                quickCollectPass()
                if QUICK_PASS_GAP_DELAY > 0 then task.wait(QUICK_PASS_GAP_DELAY) end
                if #Tracked == 0 then
                    task.wait(QUICK_EMPTY_WAIT)
                end
            end
            quickLoop = nil
        end)
