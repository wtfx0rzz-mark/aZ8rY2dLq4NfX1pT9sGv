...
return function(C, R, UI)
    ...
    local function setupItem(item)
        ...
        if hasNv then
            zr_nvConns[item] = nv.Changed:Connect(function()
                if zr_running and nv.Value ~= 0 then
                    nv.Value = 0
                end
            end)
        end
    ...
    local function stopRifleZeroReload()
        ...
        for item, conn in pairs(zr_attrConns) do
            disconnectSignal(conn)
            zr_attrConns[item] = nil
        end
        for item, conn in pairs(zr_nvConns) do
            disconnectSignal(conn)
            zr_nvConns[item] = nil
        end
    end
    ...
    --========================
    -- Auto Chest (WindUI toggle, no on-screen ON/OFF button)
    --========================
    if _G.__AutoChestStandalone and type(_G.__AutoChestStandalone.Destroy) == "function" then
        pcall(function() _G.__AutoChestStandalone.Destroy() end)
    end
    ...
    local function ensureEdgeGui()
        ...
        local oldAuto = stack:FindFirstChild("AutoChestEdge")
        if oldAuto then pcall(function() oldAuto:Destroy() end) end
        return stack
    end

    local function makeEdgeBtn(stack, name, text, order)
        local btn = stack:FindFirstChild(name)
        if not btn then
    ...
    local placeBtn = makeEdgeBtn(stack, "PlaceEdge", "Place", 951)
    ...
    function api.Destroy()
        ...
        pcall(function()
            ...
            local edgeGui = playerGui and playerGui:FindFirstChild("EdgeButtons")
            local s = edgeGui and edgeGui:FindFirstChild("EdgeStack")
            if s then
                local oldAuto = s:FindFirstChild("AutoChestEdge")
                if oldAuto then oldAuto:Destroy() end
            end
            if placeBtn then placeBtn.Visible = false end
        end)
    end

    _G.__AutoChestStandalone = api
