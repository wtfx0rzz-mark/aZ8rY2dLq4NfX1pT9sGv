...
return function(C, R, UI)
    local function run()
        ...
        local function resolveTemporalAccelerometer()
            ...
            return machine, (mp and mp.Position) or machine:GetPivot().Position
        end

        local function teleportToAccelerometer()
            local machine, pos = resolveTemporalAccelerometer()
            if not machine then return nil end
            local g = groundBelow(pos)
            local lookAt = (mainPart(machine) and mainPart(machine).Position) or pos
            local stand = Vector3.new(pos.X, g.Y + 3.0, pos.Z)
            teleportWithDive(CFrame.new(stand, lookAt))
            return machine
        end
